        ...
        if (alertsRes.ok && reportsRes.ok) {
          ...
          const activeAlerts =
            alertsData.alerts?.filter(
              (alert) =>
                alert.status === 'active' &&
                new Date(alert.createdAt).getTime() > weekAgo
            ).length || 0;
          ...
          const affectedAreas = new Set([
            ...(alertsData.alerts || []).map(
              (alert) => alert.area?.district || 'Unknown'
            ),
            ...(reportsData.reports || []).map(
              (report) => report.address?.split(',')[1]?.trim() || 'Unknown'
            ),
          ]).size;

          // Simulate response teams count
          const responseTeams = Math.max(
            3,
            Math.floor(activeAlerts * 1.5 + recentReports * 0.5)
          );
